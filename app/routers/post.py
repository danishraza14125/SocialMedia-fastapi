...
async def Get_Post(
    db: session = Depends(get_db), user_id: int = Depends(oauth2.GetCurrentUser)
):
...
async def get_one_post(
    id: int,
    db: session = Depends(get_db),
    user_id: int = Depends(oauth2.GetCurrentUser),
):
...
def delete_posts(
    ...
    if not posts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No posts found for the provided IDs",
        )
